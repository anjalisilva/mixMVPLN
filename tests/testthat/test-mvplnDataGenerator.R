...
test_that("Data generate error upon invalid user input", {
  ...
  # M is a r x p matrix
  trueM1 <- matrix(rep(6, (truer*truep)), ncol = truep, nrow = truer, byrow = TRUE)
  trueM2 <- matrix(rep(1, (truer*truep)), ncol = truep, nrow = truer, byrow = TRUE)
  trueMall <- rbind(trueM1, trueM2)
  ...
  Phi_testing <- rbind(Phi1Testing, Phi2Testing)
